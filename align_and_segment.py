...
def generate_emissions(model, audio_file):
    waveform, sample_rate = torchaudio.load(audio_file)  # waveform: channels X T

    # Set SAMPLING_FREQ to the sample rate of the audio file
    
    # Check if the sample rate is not equal to SAMPLING_FREQ, then resample
    if sample_rate != SAMPLING_FREQ:
        resampler = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=SAMPLING_FREQ)
        waveform = resampler(waveform)

    # SAMPLING_FREQ = sample_rate
    # print("Sample rate: ",audio_sf)
